function [p,X2,sigma_p,sigma_y,corr_p,R_sq,cvg_hst,secondselapsed,flag_stop,covar] = lm(func,p,t,y_dat,weight,dp,p_min,p_max,c,opts)
% [p,X2,sigma_p,sigma_y,corr_p,R_sq,cvg_hst] = lm(func,p,t,y_dat,weight,dp,p_min,p_max,c,opts)
%
...
% secondselapsed = time of executions
% flag_stop
...
%         prnt MaxIter  eps1  eps2  epx3  eps4  lam0  lamUP lamDN UpdateType MaxUnaccepted
   opts = [  3,10*Npar, 1e-3, 1e-3, 1e-3, 1e-1, 1e-2,    11,    9,        1, 500];
 end
 ...
 Update_Type   = opts(10);	% 1: Levenberg-Marquardt lambda update
                        	% 2: Quadratic update 
                            % 3: Nielsen's lambda update equations
 MaxUnaccepted = opts(11);  %Maximum number of unaccepted iterations
 ...
 plotcmd='figure(102); plot(t(:,1),y_dat,''og'',t(:,1),y_hat,''-b''); axis tight; drawnow ';
...
% initialize Jacobian with finite difference calculation
 [JtWJ,JtWdy,X2,y_hat,J,func_calls,iteration] = lm_matx(func,func_calls,iteration,t,p_old,y_old,1,J,p,y_dat,weight_sq,dp,c);
...
%  end
   ...
   if ( rho > epsilon_4 )		% it IS significantly better
	...
	y_old = y_hat;
  	p = p_try(:);			% accept p_try
        ...
        [JtWJ,JtWdy,X2,y_hat,J,func_calls,iteration] = lm_matx(func,func_calls,iteration,t,p_old,y_old,dX2,J,p,y_dat,weight_sq,dp,c);
    ...
    unaccepted_X2=unaccepted_X2+1;
    if unaccepted_X2>MaxUnaccepted
        stop = 7;
        break     
    end
    
 	if ( ~rem(iteration,2*Npar) )	% rank-1 update of Jacobian
 		[JtWJ,JtWdy,dX2,y_hat,J,func_calls,iteration] = lm_matx(func,func_calls,iteration,t,p_old,y_old,-1,J,p,y_dat,weight_sq,dp,c);
 	end
   ...
   if ( prnt > 1 )
    fprintf('>%3d:%3d | chi_sq=%15.6e | lambda=%15.6e \n', iteration,func_calls,X2,lambda );
    fprintf('    param:  ');
    for pn=1:Npar
       fprintf(' %15.6e', p(pn) );
    end
    ...
    fprintf('    dp/p :  ');
    for pn=1:Npar
       fprintf(' %15.6e', h(pn) / p(pn) );
    end
    fprintf('\n');
   end

% update convergence history
   cvg_hst(iteration,:) = [ func_calls  p'  X2/(Npnt-Npar+1) lambda ];
...
secondselapsed = toc();
if ( prnt > 0 )
    fprintf('>>> Computing Time: %.3f [sec]= %.2f[min]\n', secondselapsed,secondselapsed/60);
end
...
function [J, func_calls] = lm_FD_J(func,func_calls,t,p,y,dp,c)
% J = lm_FD_J(func,func_calls,t,p,y,{dp},{c})
%
...
function [JtWJ,JtWdy,Chi_sq,y_hat,J,func_calls,iteration] = lm_matx(func,func_calls,iteration,t,p_old,y_old,dX2,J,p,y_dat,weight_sq,dp,c)
...
%JtWdy  = zeros(Npar,1);
 ...
 if ( ~rem(iteration,2*Npar) || dX2 > 0 ) 
 	[J, func_calls] = lm_FD_J(func,func_calls,t,p,y_hat,dp,c);		% finite difference
 else
